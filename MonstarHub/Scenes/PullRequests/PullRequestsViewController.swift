...
class PullRequestsViewController: TableViewController {
    ...
    override func bindViewModel() {
        ...
        output.pullRequestSelected.drive(onNext: { [weak self] model in
            self?.viewModel.navigateTo(step: SearchStep.pullRequestDetails(value: model.repository,
                                                                           pullRequest: model.pullRequest))
        }).disposed(by: rx.disposeBag)

        output.userSelected.drive(onNext: { [weak self] user in
