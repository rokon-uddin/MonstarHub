...
class UserViewController: TableViewController {
    ...
    override func bindViewModel() {
        ...
        let dataSource = RxTableViewSectionedReloadDataSource<UserSection>(configureCell: { _, tableView, indexPath, item in
            switch item {
        ...
        output.selectedEvent.drive(onNext: { [weak self] (item) in
            ...
            case .contributionsItem:
                if let url = viewModel.skylineUrl() {
                    ...
                    viewModel.navigateTo(step: SearchStep.safari(url))
                }
            case .starsItem, .watchingItem, .eventsItem, .companyItem, .blogItem, .profileSummaryItem, .repositoryItem, .organizationItem:
        ...
        output.openInWebSelected.filterNil().drive(onNext: { url in
