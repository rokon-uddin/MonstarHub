...
class RepositoryViewModel: ViewModel, InputOutType {
    ...
    func transform(input: Input) -> Output {
        ...
        input.headerRefresh.flatMapLatest { [weak self] () -> Observable<Content> in
            ...
            let fullname = self.repository.value.fullname ?? ""
            return self.services.gitHubUseCase
                .readme(fullname: fullname, ref: nil)
                .trackActivity(self.loading)
                .trackActivity(self.headerLoading)
        ...
        let starred = input.starSelection.flatMapLatest { [weak self] () -> Observable<RxSwift.Event<Void>> in
            ...
            let request = starring == true ? self.services.gitHubUseCase
                .unstarRepository(fullname: fullname) : self.services.gitHubUseCase
                .starRepository(fullname: fullname)
            return request
                .trackActivity(self.loading)
                .materialize()
                .share()
        ...
        refreshStarring.flatMapLatest { [weak self] () -> Observable<RxSwift.Event<Void>> in
            ...
            let fullname = self.repository.value.fullname ?? ""
            return self.services.gitHubUseCase
                .checkStarring(fullname: fullname)
                .trackActivity(self.loading)
                .materialize()
                .share()
        ...
        let imageSelected = input.imageSelection.asDriver(onErrorJustReturn: ())
            .map { () -> User in
                return self.repository.value.owner ?? User()
        ...
        let items = repository.map { (repository) -> [RepositorySection] in
            ...
            let starHistoryCellViewModel = RepositoryDetailCellViewModel(with: R.string.localizable.repositoryStarsHistoryCellTitle.key.localized(),
                                                                         detail: Constants.Network.starHistoryBaseUrl,
    ...
    func stepFor(_ item: RepositorySectionItem) -> SearchStep? {
        switch item {
        case .parentItem:
            if let parentRepository = repository.value.parentRepository() {
                return .parentItem(value: parentRepository)
            }

        case .issuesItem:
            return .issuesItem(value: repository.value)

        case .commitsItem:
            return .commitsItem(value: repository.value)

        case .branchesItem:
            return .branchesItem(value: repository.value)

        case .releasesItem:
            return .releasesItem(value: repository.value)

        case .pullRequestsItem:
            return .pullRequestsItem(value: repository.value)
        ...
        case .notificationsItem:
            ...
            return .notificationsItem(value: mode)

        case .contributorsItem:
            let mode = UsersMode.contributors(repository: repository.value)
            return .contributorsItem(value: mode)

        case .sourceItem:
            if let fullname = repository.value.fullname {
                analytics.log(.source(fullname: fullname))
            }
            let ref = repository.value.defaultBranch
            return .sourceItem(value: repository.value, ref: ref)

        case .countLinesOfCodeItem:
            return .countLinesOfCodeItem(value: repository.value)
    ...
    func starHistoryUrl() -> URL? {
        return "\(Constants.Network.starHistoryBaseUrl)/#\(self.repository.value.fullname ?? "")".url
    }
}
