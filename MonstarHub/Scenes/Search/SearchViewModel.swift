...
final class SearchViewModel: ViewModel, ServicesViewModel, InputOutType {
    ...
    struct Output {
        ...
        let languagesSelection: Driver<Language?>
    ...
    let repositorySearchElements = BehaviorRelay(value: RepositorySearch(endCursor: nil))
    ...
    func transform(input: Input) -> Output {
        ...
        Observable.combineLatest(keyword, currentLanguage, sortRepositoryItem)
            ...
            .flatMapLatest({ [weak self] (_, _, sortRepositoryItem) -> Observable<RxSwift.Event<RepositorySearch>> in
                ...
                return self.services.gitHubUseCase
                    .searchRepositories(query: query, sort: sort, order: order, page: self.repositoriesPage, endCursor: nil)
        ...
        input.footerRefresh.flatMapLatest({ [weak self] () -> Observable<RxSwift.Event<RepositorySearch>> in
            ...
            return self.services.gitHubUseCase.searchRepositories(query: query, sort: sort, order: order, page: self.repositoriesPage, endCursor: endCursor)
                .trackActivity(self.loading)
        ...
        Observable.combineLatest(keyword, currentLanguage, sortUserItem)
            ...
            .flatMapLatest({ [weak self] (_, _, sortUserItem) -> Observable<RxSwift.Event<UserSearch>> in
                ...
                return self.services.gitHubUseCase.searchUsers(query: query, sort: sort, order: order, page: self.usersPage, endCursor: nil)
                    .trackActivity(self.loading)
                    .trackActivity(self.headerLoading)
                    .trackError(self.error)
                    .materialize()
        ...
        input.footerRefresh.flatMapLatest({ [weak self] () -> Observable<RxSwift.Event<UserSearch>> in
            ...
            return self.services.gitHubUseCase.searchUsers(query: query, sort: sort, order: order, page: self.usersPage, endCursor: endCursor)
                .trackActivity(self.loading)
                .trackActivity(self.footerLoading)
                .trackError(self.error)
                .materialize()
        ...
        Observable.just(()).flatMapLatest { () -> Observable<[Language]> in
            return self.services.trendingGitHubUseCase
                .languages()
                .trackActivity(self.loading)
                .trackError(self.error)
        ...
        let repositoryDetails = repositorySelected.asDriverOnErrorJustComplete()

        let userDetails = userSelected.asDriverOnErrorJustComplete()

        let languagesSelection = input.languagesSelection.asDriver(onErrorJustReturn: ()).map { () -> Language? in
