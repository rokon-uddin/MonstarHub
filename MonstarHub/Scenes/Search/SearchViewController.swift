...
final class SearchViewController: TableViewController {
...
//        bannerView.isHidden = true
    ...
    override func bindViewModel() {
        ...
        let dataSource = RxTableViewSectionedReloadDataSource<SearchSection>(configureCell: { _, tableView, indexPath, item in
            switch item {
        ...
        output.languagesSelection.drive(onNext: { _ in
            viewModel.navigateTo(step: SearchStep.languages)
        }).disposed(by: rx.disposeBag)

        output.repositorySelected.drive(onNext: { repository in
            viewModel.navigateTo(step: SearchStep.repositoryDetails(repository: repository))
        }).disposed(by: rx.disposeBag)
