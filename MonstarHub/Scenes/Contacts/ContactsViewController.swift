...
class ContactsViewController: TableViewController {
    ...
    override func bindViewModel() {
        ...
        output.contactSelected.drive(onNext: { [weak self] (contact) in
            if let strongSelf = self {
                ...
                let vc = strongSelf.toInviteContact(withPhone: phone)
    ...
    private func toInviteContact(withPhone phone: String) -> MFMessageComposeViewController {
        let vc = MFMessageComposeViewController()
        vc.body = "Hey! Come join MonstarHub at \(Constants.App.githubUrl)"
        vc.recipients = [phone]
        return vc
    }
}
