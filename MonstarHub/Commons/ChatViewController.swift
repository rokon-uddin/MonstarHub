...
class ChatViewController: MessagesViewController {
    ...
    func configureMessageInputBar() {
        ...
        messageInputBar.inputTextView.cornerRadius = Constants.Dimensions.cornerRadius
...
extension ChatViewController: MessagesDisplayDelegate {
    ...
    func configureAvatarView(_ avatarView: AvatarView, for message: MessageType, at indexPath: IndexPath, in messagesCollectionView: MessagesCollectionView) {
        if let user = message.sender as? User {
            ...
            avatarView.borderWidth = Constants.Dimensions.borderWidth
        }
    }
}
...
extension ChatViewController: AutocompleteManagerDataSource {
    ...
    func autocompleteManager(_ manager: AutocompleteManager, tableView: UITableView, cellForRowAt indexPath: IndexPath, for session: AutocompleteSession) -> UITableViewCell {
        ...
        cell.imageView?.borderWidth = Constants.Dimensions.borderWidth
...
extension Domain.User: SenderType {
    // SenderType
    public var senderId: String { return login ?? "" }
    public var displayName: String { return login ?? "" }
}

extension Domain.Comment: MessageType {
    // MessageType
    public var sender: SenderType { return user ?? User() }
    public var messageId: String { return id?.string ?? "" }
    public var sentDate: Date { return createdAt ?? Date() }
    public var kind: MessageKind { return .text(body ?? "") }
}
