...
class LibsManager: NSObject {
    ...
    let bannersEnabled = BehaviorRelay(value: UserDefaults.standard.bool(forKey: Constants.UserDefaults.bannersEnabled))

    private override init() {
        super.init()

        if UserDefaults.standard.object(forKey: Constants.UserDefaults.bannersEnabled) == nil {
            bannersEnabled.accept(true)
        }

        bannersEnabled.skip(1).subscribe(onNext: { (enabled) in
            UserDefaults.standard.set(enabled, forKey: Constants.UserDefaults.bannersEnabled)
