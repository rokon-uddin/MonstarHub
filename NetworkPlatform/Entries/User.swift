...
extension ContributionCalendar: DomainConvertibleType {
    var asDomain: Domain.ContributionCalendar {
        return Domain.ContributionCalendar(totalContributions: totalContributions,
                                           months: months.map { $0.asDomain },
                                           weeks: weeks.map { $0.map { $0.map { $0.asDomain } }})
    }
}

extension ContributionCalendar.Month: DomainConvertibleType {
    var asDomain: Domain.ContributionCalendar.Month {
        return Domain.ContributionCalendar.Month(name: name)
    }
}

extension ContributionCalendar.ContributionDay: DomainConvertibleType {
    var asDomain: Domain.ContributionCalendar.ContributionDay {
        return Domain.ContributionCalendar.ContributionDay(color: color,
...
extension User: DomainConvertibleType {
    var asDomain: Domain.User {
        return Domain.User(avatarUrl: avatarUrl,
                           blog: blog,
                           company: company,
                           ...
                           login: login,
                           name: name,
                           type: Domain.UserType(rawValue: type.rawValue)!,
                           updatedAt: updatedAt,
                           ...
                           isViewer: isViewer,
                           pinnedRepositories: pinnedRepositories?.map { $0.asDomain },
                           organizations: organizations.map { $0.asDomain },
                           contributionCalendar: contributionCalendar?.asDomain)
    }
}
...
extension UserSearch: DomainConvertibleType {
    var asDomain: Domain.UserSearch {
        return Domain.UserSearch(items: items.map { $0.asDomain },
...
extension TrendingUser: DomainConvertibleType {
    var asDomain: Domain.TrendingUser {
        return Domain.TrendingUser(username: username,
                                   name: name,
                                   url: url,
                                   avatar: avatar,
                                   repo: repo?.asDomain,
                                   type: Domain.TrendingUserType(rawValue: type.rawValue)!)
    }
}
